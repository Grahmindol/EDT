(async function main() {
	...
	ScheduleTemplate.prototype.openModal = function (target) {
		...
		setTimeout(function () {
			...
			Util.addClass(target.closest('li') || document.createElement("li"), 'cd-schedule__event--selected');
		}, 10);
	...
	ScheduleTemplate.prototype.closeModal = function () {
		...
		var item = self.element.getElementsByClassName('cd-schedule__event--selected')[0] || document.getElementById("parameter_button"),
			target = item.getElementsByTagName('a')[0] || document.getElementById("parameter_button");
		...
		httpRequest.onreadystatechange = function () {
			if (httpRequest.readyState === XMLHttpRequest.DONE) {
				if (httpRequest.status === 200) {
					...
					// Execute inline scripts using eval()
					var scripts = self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].getElementsByTagName('script');
					for (var i = 0; i < scripts.length; i++) {
						if (!scripts[i].src) {
							// If it's an inline script, use eval to execute its content
	...
	function actualise() {
		...
		document.getElementById("parameter_button").addEventListener('click', function (event) {
			event.preventDefault();
			if (!scheduleTemplateArray[0].animating) scheduleTemplateArray[0].openModal(document.getElementById("parameter_button"));
		});
	}
	...
	if (scheduleTemplate.length > 0) { // init ScheduleTemplate objects
		...
		function loadCustomColor() {
			...
			for (let index = 0; index <= 5; index++) {
				if (localStorage.getItem(`event-${index}-color`)) {
					root.style.setProperty(`--event-${index}-color`, localStorage.getItem(`event-${index}-color`));
				}
			}
		}
	}
